...
export const createServer = (price: number) => {
    ...
    server.paidTool("query_dataset", QUERY_DATASET_DESCRIPTION, {
        ...
        eventSignature: z.string().optional().describe("The event signature to filter for. If you don't have this, use the fetch_abi tool. Needs to be the full signature, like: PairCreated(address indexed token0, address indexed token1, address pair, uint) or PoolCreated(address indexed token0, address indexed token1, uint24 indexed fee, int24 tickSpacing, address pool)."),
